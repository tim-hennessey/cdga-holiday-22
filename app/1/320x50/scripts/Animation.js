...
app.Animation = (function () {
	...
	function initialize() {
		...
		t.set(can, {transformOrigin:"80% 0%", y:h, rotation: -20});
	...
	function start() {
		...
		.to(txt2, 1, {y:0, autoAlpha:1, ease: Sine.easeOut}, "-=.75")
		.to(txt2, 1, {y:-50, autoAlpha:0, ease: Sine.easeIn}, "+=2")

		.to(txt3, 1, {y:0, autoAlpha:1, ease: Sine.easeOut}, "-=.75");
        ...
        function animm1(elm) {
            t.to(elm, R(.75, 1), {y: 0, ease: Linear.easeNone, repeat: -1, delay: -5});
            t.to(elm, R(.1, .5), {x:R("+=1","+=5"),ease: Sine.easeInOut, yoyo:true, repeat: -1, delay: -5});
        }

        function animm2(elm) {
            t.to(elm, R(.5, .75), {y: 0, ease: Linear.easeNone, repeat: -1, delay: -5});
            t.to(elm, R(.1, .5), {x:R("+=1","+=5"),ease: Sine.easeInOut, yoyo:true, repeat: -1, delay: -5});
        }
